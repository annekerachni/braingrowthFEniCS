...
class GrowthTensor:
    ...
    def __init__(self):
        ...
        print("initializing growth tensor...")

        # tangential growth coefficient 
        dgTANCortex0 = 0. # Here: growth variation is between mesh t-dt and mesh t. In original BrainGrowth: the growth variation is between mesh 0 and mesh t
        dgTANCore0 = 0.
        self.dg_cortex_TAN = fenics.Expression('dgTANCortex', degree=1, dgTANCortex=dgTANCortex0) # Here: g_TAN[Point] = 1 + dg_TAN[Point] = = 1 + gr_braingrowthFEniCS*alpha_TAN[Point]*dt. In original BrainGrowth: g_TAN[Point] = 1 + at_TAN[Point] * gm_TAN[Point] = 1 + alpha_TAN[Point]*t * gr_BrainGrowth_TAN[Point]/(1 + exp**(10(Point.y/cortical_thickness - 1)))
        self.dg_core_TAN = fenics.Expression('dgTANCore', degree=1, dgTANCore=dgTANCore0)

        # radial growth coefficient
        dgRADCortex0 = 0.
        dgRADCore0 = 0.
        self.dg_cortex_RAD = fenics.Expression('dgRADCortex', degree=1, dgRADCortex=dgRADCortex0)
        self.dg_core_RAD = fenics.Expression('dgRADCore', degree=1, dgRADCore=dgRADCore0)
    ...
    def define_adaptative_growth_tensor(self, u, Mesh_Nt, dg_TAN, dg_RAD): 
        ...
        Fg =  (1 + dg_TAN) * ( Id - Nt_Nt_proj ) + (1 + dg_RAD) * Nt_Nt_proj 
