...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', help='Input mesh path (xml)', type=str, required=False, 
                        default='./data/fetal_dhcp_atlas/21GW/dhcp21GW_708Ktets.xml') 
    ...
    parser.add_argument('-seg', '--segmentationnifti', help='Path to the fetal dchp atlas Segmentation niftis at 21GW', type=str, required=False, 
    default='/home/anne/Bureau/fetal_dhcp_atlas/tissue-t21.00_dhcp-19.nii.gz')
    
    parser.add_argument('-fa', '--fractionalanisotropynifti', help='Path to the fetal dchp atlas FA niftis at 21GW', type=str, required=False, 
    default='/home/anne/Bureau/fetal_dhcp_atlas/fa-t21.00.nii.gz')
    ...
    parser.add_argument('-p', '--parameters', help='Simulation input parameters', type=json.loads, required=False, 
                        default={#"H0": 0.04, 
                                 ...
                                 "rho": 0.01, 
                                 "damping_coef": 10.0,
                                 "linearcoef": 5.0, # alphaTAN = linearcoef * FA
                                 #"alphaTAN": 5.0, "alphaRAD": 0.0, "grTAN": 1.0, "grRAD": 1.0, 
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./braingrowth/results/solverFg0_FA_bilayers/dhcp21GW708Ktets_K100_muCortex20_muCore4_rho001_linearcoefFA5_contactGlobalSmallCorrections_abs3re2/')
    ...
    # 1. Load Segmentation labels and FA from MRI nifti at T0 (e.g. 22GW)
    # ---
    FA_nifti_file_path_T0GW = args.fractionalanisotropynifti
    Segmentation_nifti_file_path_T0GW = args.segmentationnifti
    
    interpolation_mode = 'nearest_neighbor'
    
    # mesh should not be normalized: 
    mesh0 = fenics.Mesh(args.input)
    ...
    mesh_meshio = transfer_niftivalues_to_meshnodes_in_simulation.load_MRIdata_onto_mesh_nodes(mesh0, FA_nifti_file_path_T0GW, Segmentation_nifti_file_path_T0GW, interpolation_mode) # meshio mesh 
    mesh_meshio = transfer_niftivalues_to_meshnodes_in_simulation.correct_segmentation_label(mesh_meshio)
    mesh_meshio.write(args.output + "mesh_MRIniivalues_{}GW".format(T0) + '.vtk') 
    
    
    # 2. Defining gr from Cortex segmentation label
    # ---
    ...
    # 3. Defining tangential growth coefficient 'alphaTAN' in correlation to FA nodal value. (FA ++ => neuronal maturation -- => neuron matures => tangential growth ++)
    # ---
    ...
    fa, normalized_fa = FA_to_growth_longitudinal.normalize_FA(fa, normalized_fa, vertex2dofs_S, mesh_meshio)
    
    # Export normalized_FA 
    normalizedFA_file_XDMF = fenics.XDMFFile(args.output + "normalizedFA_{}.xdmf".format(T0))
    ...
    alphaTAN = FA_to_growth_longitudinal.tangential_growth_coef_from_FA(linear_coef, alphaTAN, vertex2dofs_S, normalized_fa)
    
    # Export tangential growth coefficient 'alphaTAN'
    alphaTAN_file_XDMF = fenics.XDMFFile(args.output + "alphaTAN_{}.xdmf".format(T0))
    ...
    # 4. TANGENTIAL growth
    # ---
    dg_TAN.assign( fenics.project(grTAN * grNoGrowthZones * alphaTAN * ((1 - alphaF) * dt), S) )
    
    dgTAN_file_XDMF = fenics.XDMFFile(args.output + "dgTAN_{}.xdmf".format(T0))
    ...
    result_file.write(grNoGrowthZones, T0)
