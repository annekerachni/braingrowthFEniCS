...
def msh_to_stl(input_file_msh, output_file_stl): 
    """
    ...
    # get .xml mesh in order to get bmesh and to finally get surface points
    for cell in mesh.cells:
        if cell.type == "tetra": 
            tetra_mesh = meshio.Mesh(points=mesh.points, cells={"tetra": cell.data})
            meshio.write(output_file_stl.split('.stl')[0] + ".xml", tetra_mesh)
    
    FEniCS_mesh = fenics.Mesh(output_file_stl.split('.stl')[0] + ".xml")
    FEniCS_bmesh = fenics.BoundaryMesh(FEniCS_mesh, "exterior") 

    surface_points = FEniCS_bmesh.coordinates()
    triangles = FEniCS_bmesh.cells()

    # export .stl
    meshio.write(output_file_stl, meshio.Mesh(points = surface_points, cells = {'triangle': triangles})) 
...
if __name__ == '__main__':
    ...
    if inputmesh_format == 'vtk':
        ...
        if outputmesh_format == 'stl': # Netgen mesh
