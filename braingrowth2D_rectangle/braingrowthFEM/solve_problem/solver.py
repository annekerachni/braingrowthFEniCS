...
from braingrowth2D_rectangle.braingrowthFEM.solve_problem import mappings
...
class NonLinearDynamicMechanicsSolver: 
    ...
    def set_solver_parameters(self, 
        ...
        prm['newton_solver']['preconditioner'] = linearsolver_preconditionner # "Incomplete LU factorization" --> "It conditions a given problem into a form that is more suitable for numerical solving methods". Preconditionner enables to reduce computationnal cost. https://computationalmechanics.in/preconditioners/
        
        if linearsolver_preconditionner != None: # i.e. if linear solver is iterative (Krylov subspace method ("PETSc Krylov solver") --> it is required to specify a preconditionner). Else, linear solver is direct (e.g. mumps), well suitable for low range meshes (low n_nodes).
