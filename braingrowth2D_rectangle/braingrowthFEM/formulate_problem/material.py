...
class NeoHookeanElasticMaterial(Material):
    ...
    def __init__(self, shared_parameters, kinematics, neohookean_specific_parameters):

        super().__init__(shared_parameters)

        self.kinematics = kinematics

        self.neohookean_specific_parameters = neohookean_specific_parameters
        self.allocate_specific_parameters()
    

    def allocate_specific_parameters(self):
        self.mu = self.neohookean_specific_parameters['mu']
        self.k = self.neohookean_specific_parameters['k']
    ...
    def compute_Cauchy_stress(self):
        ...
        """
        self.Te = self.mu * (self.kinematics.Be - (self.kinematics.Tre)/3 * self.kinematics.Id) * pow(self.kinematics.Je, -5/3) \
                + self.k * (self.kinematics.Je - 1) * self.kinematics.Id # X.Wang (originel BrainGrowth simulation code)
