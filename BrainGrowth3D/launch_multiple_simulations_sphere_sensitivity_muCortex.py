...
if __name__ == '__main__':
    ...
    ##################
    H0_values = [1.5e-3] # [m] --> 5% of brain radius (30 mm)
    muCortex_values = [1500, 3000, 4500, 6000] # [Pa]
    nu_min_max = [0.45] # [-]
    alphaTAN_min_max = [4.0e-7] # [s⁻¹]
    dt_min_max = [86400] # [s] --> [1800, 3600, 43200, 86400]

    # input mesh
    ############
    mesh_inputpath = './data/sphere/sphere_257Ktets_20Kfaces_refined5.xdmf'  # sphere mesh in meters
            ...
            for nu in nu_min_max:
                for alphaTAN in alphaTAN_min_max : 
                    for dt in dt_min_max:
                        ...
                        output_path = "./results/sphere_growth_sensititivy_analysis_muCortex/sphere257Ktets_H{}_muCortex{}_muCore{}_nu{}_alphaTAN{}_dt{}/".format(str(H0).replace('.', "_"), str(muCortex), str(muCore), str(nu).replace('.', "_"), str(alphaTAN).replace('.', "_").replace('-', "_"), str(dt))
                    
                        simulations.append("""python BrainGrowth3D/main_sphere_growth.py -i """ + mesh_inputpath + """ -c True -p '{"H0": """ + str(H0) + """, "muCortex": """+ str(muCortex) + """, "muCore": """ + str(muCore) + """, "nu": """ + str(nu) + """, "alphaTAN": """ + str(alphaTAN) + """, "alphaRAD": 0.0, "grTAN": 1.0, "grRAD": 1.0, "T0_in_GW": 21.0, "Tmax_in_GW": 36.0, "dt_in_seconds": """ + str(dt) + """, "linearization_method":"newton", "newton_absolute_tolerance":1E-9, "newton_relative_tolerance":1E-6, "max_iter": 15, "linear_solver":"mumps"}' -o """ + output_path) 
