...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./results/brain_growth_TwoRigidPlanes_penaltyMethodCorrected_BoundaryNt101_103_104_dirichletEllipsoidVZ/') 
    ...
    for cell in fenics.cells(mesh_WholeBrain):
        ...
        if zone_to_cut_scalar_FEM_function(point) <= 0:  
            cell_markers[cell] = 10
    # to check labelling of the VZ zone: cell_markers.array(); np.where(cell_markers.array() == 10)
            
    # get submeshes
    ellipsoid_VZ_mesh = fenics.SubMesh(mesh_WholeBrain, cell_markers, 10)
    mesh = fenics.SubMesh(mesh_WholeBrain, cell_markers, 0)
    ...
    # ---
    print("\nmarking the contact boundaries (right: 103 (Y+) since inverted by Netgen; left: 104 (Y-))...")
    ...
    for facet in fenics.facets(mesh):
        if facet.exterior() == True: # to select only faces at the exterior surface of the mesh (otherwise, inner faces are also marked)
            ...
            if Y_inner_facet <= y_interhemisphere_plane_103 + tol and Y_inner_facet <= abs(y_interhemisphere_plane_104 - tol): # facet in the interhemispheric zone, between the two rigid planes. This facet should not grow, and should not be taken as reference to build Mesh_Nt.
    ...
    for facet in fenics.facets(mesh):
        if facet.exterior() == True: # to select only faces at the exterior surface of the mesh (otherwise, inner faces are also marked)
            ...
            if Y_inner_facet <= y_interhemisphere_plane_103 + tol and Y_inner_facet >= y_interhemisphere_plane_104 - tol: # facet in the interhemispheric zone, between the two rigid planes. This facet should not grow, and should not be taken as reference to build Mesh_Nt.
               if volume_facet_marker.array()[facet.index()] == 102:
                   pass
               else:
                   volume_facet_marker.array()[facet.index()] = 99
