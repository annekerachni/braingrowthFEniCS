...
############################
output_folder = "./contact_mechanics/results/cube_Eulerian_hyperelastic/"
...
# Input mesh
############
mesh = fenics.UnitCubeMesh(10, 10, 10)


# Initial coordinates of the spheremesh
X = fenics.SpatialCoordinate(mesh) # reference configuration 
...
radius_Z = 0.5 * (np.max(mesh.coordinates()[:,2]) - np.min(mesh.coordinates()[:,2]))
...
fixed_displacement_zone = fenics.CompiledSubDomain('on_boundary && x[2] > fixed_height && x[2] < cube_height+tol', tol=fenics.DOLFIN_EPS, fixed_height=1.9*radius_Z, cube_height=2*radius_Z) # Contact search - contact part of the boundary
...
V = fenics.VectorFunctionSpace(mesh, "CG", 2) # CG, 2 to have enought continuity to get the approximated result for unknown displacement
#T = fenics.TensorFunctionSpace(mesh,'CG', 1, shape=(3,3))
...
K = E * mu_ / (3 * (3 * mu_ - E)) # N.m⁻² = Pa

# Contact coefficient
# -------------------
#epsilon = fenics.Constant(1e4) 
#h = mesh.hmin() 
h = 0.005 # since max displacement is 0.01 with no Contact Mechanics added in residual form
...
We = 0.5 * mu_ * (Tre * pow(Je, -2/3) - 3) + 0.5 * K * (Je - 1) * (Je - 1) # X.Wang https://github.com/rousseau/BrainGrowth/
...
PK1e_NH = fenics.diff(We, Fe) 
sigma_NH = Fe * PK1e_NH.T / Je # Nanson's formula
...
def normal_gap(u, z_plane): # Definition of gap function (if gap < 0 => penalization)
    ...
    return x[2] + u[2] - z_plane # (= MS.n) compute each time new virtual configuration at t+1  
    # gn = MS.n => -gn = SM.n = z_plane - (x[2] + u[2])
...
#res_form = fenics.inner(PK1e_NH, fenics.grad(v_test)) * fenics.Measure("dx") - fenics.dot(b, v_test) * fenics.Measure("dx") - epsilon * fenics.dot(mackauley( -normal_gap(u, z_plane) ) * n, v_test) * ds(1) 
#res_form = fenics.inner(sigma_NH, fenics.grad(v_test)) * fenics.Measure("dx") - fenics.dot(b, v_test) * fenics.Measure("dx") - epsilon * fenics.dot(mackauley( -normal_gap(u, z_plane) ) * n, v_test) * ds(1) 
res_form = fenics.inner(sigma_NH, fenics.grad(v_test)) * fenics.Measure("dx") \
...
# Export results
################
file_results = fenics.XDMFFile( os.path.join(output_folder, "results_cube_contact_SIGMA_Contact_Hyperelastic.xdmf") )
