...
def compute_mesh_projected_normals(V, mesh_vertex_coords, bmesh_vertex_coords, vertexCortexSurface_2_dofsV_mapping, vertex2dofs_V, Boundary_Nt):
    ...
    for vertex_index, coords in enumerate(mesh_vertex_coords): #N vertex indices in the whole Mesh ref
        ...
        # Compute vertex index (int) in cortex surface (Function Space) reference (i.e. less indices) of the closest Point at Top Boundary to given Mesh Point ('vertex_index')   
        closestCortexSurfacePointIndex_inCortexSurfaceref = np.argmin(np.linalg.norm(bmesh_vertex_coords - mesh_vertex_coords[vertex_index], axis=1)) # index in the cortex surface bmesh for 2D closest Point/Vertex
        closestCortexSurfacePointDOF_inWholeMeshref = vertexCortexSurface_2_dofsV_mapping[closestCortexSurfacePointIndex_inCortexSurfaceref]
        ...
        Mesh_Nt.vector()[vertex2dofs_V[vertex_index]] = Boundary_Nt.vector()[closestCortexSurfacePointDOF_inWholeMeshref] 
