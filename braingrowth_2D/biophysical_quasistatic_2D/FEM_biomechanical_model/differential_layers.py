...
def compute_distance_to_cortexsurface(vertex2dof_S, d2s_S, mesh, bmesh_cortexsurface_bbtree):
    ...
    for idx, x in enumerate(mesh.coordinates()): # point: idx (x[0]), coords (x[1])
        ...
        d2s_S.vector()[vertex2dof_S[idx]] = distance_to_cortexsurface_x
        
    return d2s_S

"""
def compute_distance_to_cortexsurface_2(vertex2dof_S, d2s_S, mesh, bmesh_cortexsurface_tree):
...
def compute_differential_term(S, d2s_S, H_S, gm_S): # gm
