...
def spectrum(f2analyse, mass_matrix, eig_vec, eig_val):
    ...
    """
    # grouped_spectrum: mean frequency for each local dominant band
    # group_indices: range of eigenvalue order for each local dominant band (e.g. [0 0] [4 17] ... [  N-1])
    # coefficients: coefficient resulting from the projection of the neurofunctional signal to analyse onto the LBO eigenpairs basis. Coeffcients correspond to decompostion of the whole brain neurofunction spectrum. number of coefficients = N 
    # nlevels: number of compacted frequency bands. Increase with number of wished eigenpairs (and also size of the mesh?)

    coefficients = f2analyse.dot(mass_matrix.transpose().dot(eig_vec))
    nlevels = int(0.5 * np.log(eig_val[-1] / eig_val[1]) / np.log(2)) # nlevels = 5 --> ?
    grouped_spectrum = np.zeros((nlevels + 2, 1))
    grouped_spectrum[0] = coefficients[0]**2
    group_indices = np.zeros((nlevels + 2, 2), dtype=int)
    group_indices[0, :] = [0, 0]

    for k in range(nlevels): # 0 1 2 3 4
        indice = np.where(eig_val >= eig_val[1] * 2 ** (2 * (k)))
        group_indices[k + 1, 0] = indice[0][0] # 1 2 3 4 5 
    ...
    group_indices[-1, 0] = group_indices[-2, 1] + 1 # 6 
...
def spectrum_reindexed(f2analyse, mass_matrix, eig_vec, eig_val):
    """Compute the (grouped) spectrum of a scalar function  in a functional
    basis
    The functional basis is derived from the eigenvectors of a laplace
    beltrami operator defined on the mesh. The spectrum function:
    ...
    for k in range(nlevels-2): # 0 1 2 3 4
        indice = np.where(eig_val >= eig_val[1] * 2 ** (2 * (k)))
        group_indices[k + 1, 0] = indice[0][0] # 1 2 3 4 5 
        indice = np.where(eig_val <= eig_val[1] * 2 ** (2 * (k + 1)))
        group_indices[k + 1, 1] = indice[0][-1]
        grouped_spectrum[k + 1] = \
            np.sum(coefficients[
                   group_indices[k + 1, 0]:group_indices[k + 1, 1] + 1]**2) # sum of all squared projection coef belonging to the band range (e.g. B6 --> sum(coefs_i²) corresponding to all eigenvalue_orders_i within this band) 
    ...
    return grouped_spectrum.squeeze(), group_indices, coefficients, nlevels # group_spectrum ~ big compacted projection coefficient
...
def local_dominance_map(
    ...
    frecomposed = np.zeros((len(f2analyse), nlevels - 1), dtype='object') # nlevels - 1 = 6
