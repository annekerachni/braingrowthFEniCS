...
class Mapping:
    ...
    def __init__(self, mesh, brainsurface_bmesh, VectorSpace_CG1_mesh, VectorSpace_CG1_bmesh):
        self.mesh = mesh
    ...
    def boundaryFunctionSpace_2_meshFunctionSpace_dofmap(self):
        ...
        # find (redundant) coupled-DOFsV corresponding to each single DOFB
        ...
        # Or self.B_2_V_dofmap should be np.array([[8808, 8809], [8810, 8811], [8806, 8807], ...]) corresponding coupled-DOFsV to following coupled-DOFB:  np.array([[0, 1], [2, 3], [4, 5],  ...])
    ...
    def boundaryFunctionSpace_2_meshFunctionSpace_dofmap_V1(self):
        ...
        bmsh_dof_coordinates = self.VectorSpace_CG1_bmesh.tabulate_dof_coordinates()
        mesh_dof_coordinates = self.VectorSpace_CG1_mesh.tabulate_dof_coordinates() 
    ...
    """ 
    def boundaryFunctionSpace_2_meshFunctionSpace_dofmap_V2(self):
        # code source: https://fenicsproject.org/qa/8522/mapping-degrees-of-freedom-between-related-meshes/
        print("creating mapping from the brainsurface boundary function space to the whole mesh function space...")

        # make a map of dofs from the boundarymesh to the original mesh
        bmsh_dof_coordinates = self.VectorSpace_CG1_bmesh.tabulate_dof_coordinates() #L.dofmap().tabulate_all_coordinates(left_mesh).reshape(-1, gdim)
        mesh_dof_coordinates = self.VectorSpace_CG1_mesh.tabulate_dof_coordinates() #V.dofmap().tabulate_all_coordinates(mesh).reshape(-1, gdim)
        
        self.B_2_V_dofmap = {}
        One_ofPointVDOFs_already_checked = {}

        for Bdof_nr, Bdof_coords in enumerate(bmsh_dof_coordinates):
            corresponding_Vdof = [i for i, Vdof_coords in enumerate(mesh_dof_coordinates) if np.array_equal(Vdof_coords, Bdof_coords)] # 2 dofs associated to one Point since we use VectorFunctionSpaces V and B
            point = tuple(Bdof_coords)
            if point not in One_ofPointVDOFs_already_checked: # if key "point" does not exist yet
                self.B_2_V_dofmap[Bdof_nr] = corresponding_Vdof[0] # allocate first Vdof of the Point
                One_ofPointVDOFs_already_checked[point] = 1
