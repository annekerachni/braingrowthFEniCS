...
from FEM_biomechanical_model_quasistatic import preprocessing, mappings, differential_layers, growth, projection
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='braingrowthFEniCS: halfbrain growth quasistatic 3D model')

    parser.add_argument('-i', '--input', help='Input mesh path (xml)', type=str, required=False, 
                        default='./data/halfbrain/dHCP_21GW/XML_XDMF/fetal_week21_right_pial_surf_Y_35000faces_176000tets_refinedWidthCoef10.xdmf') 

    parser.add_argument('-p', '--parameters', help='Simulation input parameters', type=json.loads, required=False, 
                        default={"H0": 2.0e-3, # [m] 
                                 "muCortex": 300, "muCore": 50, # [Pa]
                                 "nu": 0.45,
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./results/halfbrain_reorientedY_Fgt_quasistatic_biophysical/')
    ...
    # Reoriented mesh and update bmesh and characteristics
    # ----------------------------------------------------
    print("\nreorienting mesh + placing COG at origin...")
    mesh = preprocessing.reorient_mesh(mesh, characteristics0, center_of_gravity0) # since netgen change x<>y axis compared to .stl orientation
    mesh = preprocessing.converting_mesh_from_millimeters_into_meters(mesh) # mesh lengths in meters (SI)
    ...
    boundaries_volume = fenics.MeshFunction("size_t", mesh, mesh.topology().dim() - 1) # dimension of facets --> mesh.num_facets()
    boundaries_volume.set_all(100)
    
    boundaries_surface = fenics.MeshFunction('size_t', bmesh, bmesh.topology().dim(), 0)
    
    # define limit zone (// z) where boundary does not belong to cortex
    dz = characteristics['maxz'] - characteristics['minz']
    interhemisphere_margin = dz / 5

    # define cortex surface
    class CortexSurface(fenics.SubDomain): 
        def __init__(self, interhemisphere_margin):
            fenics.SubDomain.__init__(self)
            self.interhemisphere_margin = interhemisphere_margin

        def inside(self, x, on_boundary):  
            tol = 1E-14
            return x[2] <= -self.interhemisphere_margin + tol # need to remove "on_boundary" to build submesh of a boundary mesh --> https://fenicsproject.discourse.group/t/how-to-build-vectorfunctionspace-on-part-of-a-2d-mesh-boundary/9648; returns all boundary points except those on interhemisphere plane, supposed to be fixed.

    # mark cortex surface
    cortexsurface = CortexSurface(interhemisphere_margin)
    ...
    # build cortical boundary mesh
    bmesh_cortex = fenics.SubMesh(bmesh, boundaries_surface, 101) # part of the boundary mesh standing for the cortical surface
    ...
    # Define brain regions that grow (label grGrowthZones to apply to growth ratio)
    # ------------------------------  
    
    # Regions that do not grow are defined with an ellipsoid inside the halfbrain volume and associated with the rqp coefficient
    """
    print("mark no-growth brain regions (e.g. 'longitudinal fissure' - 'ventricules' - 'mammilary bodies')...") # reference: T.Tallinen et al. 2016. See detail in https://github.com/rousseau/BrainGrowth/blob/master/geometry.py   
    ...
    new_critical_value = (pond_Y - H0) # 0.6 * pond_Y. initial value from T.Tallinen for delimating growth zones from inner zones: 0.6
    ...
    # Regions that do not grow are defined as a pave in the inner halfbrain, near by the interhemisphere
    for vertex, scalarDOF in enumerate(vertex2dofs_S):
    ...
    # define part of the boundary to fix (Dirichlet BCs)  --> https://fenicsproject.org/qa/2989/vertex-on-mesh-boundary/
    class InterHemisphereZone(fenics.SubDomain): 
        def __init__(self, interhemisphere_margin):
            fenics.SubDomain.__init__(self)
            self.interhemisphere_margin = interhemisphere_margin
        ...
        def inside(self, x, on_boundary): 
            tol = 1E-14
            return x[2] > -self.interhemisphere_margin and on_boundary
    
    # mark boundaries to fix
    interHemisphereZone = InterHemisphereZone(interhemisphere_margin)
    ...
    #Mesh_Nt.assign( growth.compute_mesh_projected_normals(V, mesh.coordinates(), bmesh.coordinates(), vertexB101_2_dofsV_mapping, vertex2dofs_V, BoundaryMesh_Nt) )
    ...
    for i, dt in enumerate( tqdm( np.diff(times), desc='brain is growing...', leave=True) ): # dt = dt_in_seconds
        ...
        bmesh_cortex = fenics.SubMesh(bmesh, boundaries_surface, 101)
