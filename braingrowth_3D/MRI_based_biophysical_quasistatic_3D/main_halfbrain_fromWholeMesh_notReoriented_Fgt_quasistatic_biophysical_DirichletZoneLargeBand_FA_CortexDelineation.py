...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--parameters', help='Simulation input parameters', type=json.loads, required=False, 
                        default={#"H0": 2.25e-3, # [m]
                                 ...
                                 "nu": 0.45,
                                 #"alphaTAN": 5.0e-6, "grTAN": 1.0,
                                 "FA_to_alphaTAN_coef": 10.0e-6, # FA_to_alphaTAN_coef: [s⁻¹]
                                 "alphaRAD": 0.0, "grRAD": 1.0, # alphaTAN: [s⁻¹]
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./results/halfbrain_fromWholeMesh_notReoriented_Fgt_quasistatic_alphaTANFAbased_H0segmentation/')
    ...
    volume_subdomains.array()[vertices_Cortex[:]] = 1
    """
    ...
    for tet in fenics.cells(mesh):
        ...
        if grTAN.vector()[vertex2dofs_S[vertex1]] == 1. and \
           ...
           volume_subdomains.array()[tet.index()] = 1
    """
    ...
    #export_XML_PVD_XDMF.export_PVDfile(args.output, 'volume_subdomains_T0', volume_subdomains) 

    # Measurement entities 
    # --------------------
    ds = fenics.Measure("ds", domain=mesh, subdomain_data=boundaries_volume) 
    #dx = fenics.Measure("dx", domain=mesh, subdomain_data=volume_subdomains) # Volume measurement entities 
