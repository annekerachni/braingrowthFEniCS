...
from utils.mesh_generators import create_xml2Dmesh_withFEniCS
from braingrowth2D_rectangle.braingrowthFEM.input_mesh import geometry
from braingrowth2D_rectangle.braingrowthFEM.formulate_problem import problem
from braingrowth2D_rectangle.braingrowthFEM.solve_problem import solver
...
mesh_folderpath = './braingrowth2D_rectangle/data/'
geometry_type = 'rectangle'
geometry_name = 'rectangle_1' # name that will be provided to the mesh file
mesh_parameters = {'xmin': 0., 'xmax': 1., 'ymin': 0., 'ymax': 2., 'num_cell_x': 60, 'num_cell_y': 60}
brain_representation = 'partial' # 'partial' if mesh does not represent the full brain (e.g. rectangle, halfdisk, quarterdisk); 'whole' otherwise (e.g. in 2D: disk; in 3D: ellipsoid, dhcp, tallinen)
createdmesh = create_xml2Dmesh_withFEniCS.RectangleMesh(mesh_folderpath, geometry_name, **mesh_parameters)
mesh = createdmesh.mesh
vedo.dolfin.plot(mesh, wireframe=False, text='mesh', style='paraview', axes=4).close()
meshpath = createdmesh.path # .xml mesh file
...
# ------------------------------------
dirichlet_bcs_parameters = {'consider_brainsurface_bc_TrueorFalse': False, 'brainsurface_bc': None, # e.g. 'consider_brainsurface_bc_TrueorFalse': True + 'brainsurface_bc' = fenics.Constant(0., -0.1); fenics.Expression(...)
                            'consider_left_bc_TrueorFalse': True, 'left_bc_type': 'plan_rolling', # 'left_bc_type': "fixed"; "plan_rolling"
                            'consider_right_bc_TrueorFalse': True, 'right_bc_type': 'plan_rolling', # 'right_bc_type': "fixed"; "plan_rolling"
                            'consider_bottom_bc_TrueorFalse': True, 'bottom_bc_type': 'fixed'} # 'bottom_bc_type': "fixed"; "plan_rolling"
...
core_material_parameters = {'mu': 1., # # 1. for rectangle mesh / 1.167 in original BrainGrowth
                            'k': 100.} # 100. for rectangle mesh / 5. for in original BrainGrowth
...
braingrowth_problem = problem.NonLinearDynamicMechanicsProblem(meshObj, 
                                                               ...
                                                               body_forces,
                                                               results_folderpath='./braingrowth2D_rectangle/results/')
