...
def apply_lps_ras_transformation(reference_image_itk): 
    ...
    return reference_image_itk # reference image is now in RAS+ 


import argparse
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Transform nifti image from LPS+ (itk) to RAS+')
    parser.add_argument('-i', '--input', help='Path to input nifti image in LPS+', type=str, required=False, default='./data/dhcp/dhcp0.nii')
    args = parser.parse_args()

    reference_nii_path = args.input 
    ...
    origin0, spacing0, d0 = nifti_itk_analyzer(reference_image_itk)
    print("initial orientation of the image: {}".format(orientation0))
    print("direction cosine matrix:\n{}".format(d0))
    print("origin coordinates: {}".format(origin0))
    print("resolution: {}".format(spacing0))
    ...
    print('\n')
    print("orientation of the image after RAS+ transformation: {}".format(orientation_ras))
    print("direction cosine matrix after RAS+ transformation:\n{}".format(d_ras))
    print("origin coordinates after RAS+ transformation: {}".format(origin_ras))
    print("resolution after RAS+ transformation: {}".format(spacing_ras))
