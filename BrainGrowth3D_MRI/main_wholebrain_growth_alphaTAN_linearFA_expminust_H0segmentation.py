...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./results/ECCOMAS_57/wholebrain/9SEPT/dHCPsurface21GW_wholebrain_Fgt_quasistatic_alphaTANFAbased_linearFA_expminust_H0segmentation_DIRICHLETboundaries_GAP_dgRAD_calibration/')
    ...
    cortex_submesh.mark(subdomains, 3)
    """
    ...
    ####
    ####
    # Compute the volume of the cortex brain mesh 
    ...
    # Compute mean thickness of the cortical layer (to be used as the value in the homogenous case for results to be compared)
    ...
    alphaTAN_normalizedFA.assign( load_mesh_with_growthcoef_ECCOMAS_XDMF.tangential_growth_coef_from_linear_FA(alphaTAN_normalizedFA, 
                                                                                                                      vertex2dofs_S, 
    ...
    list_alphaTAN_linearFA_normalizedFAbased = [] # Sum(alphaTAN_FA)_{n_nodes_in_Cortex}
    for scalarDOF in list_of_scalarDOF_in_Cortex:
        list_alphaTAN_homogeneous.append(alphaTAN_homogeneous * grTAN.vector()[scalarDOF])
        list_alphaTAN_linearFA_normalizedFAbased.append( alphaTAN_normalizedFA.vector()[scalarDOF] ) 
    
    coef_lissage_alphaTAN_FA = np.sum(list_alphaTAN_homogeneous) / np.sum(list_alphaTAN_linearFA_normalizedFAbased)
    ...
    # collisions (fcontact_global_V) have to be detected at each step
        ...
        projection.local_project((1 - grTAN) * alphaRAD * dt, S, dg_RAD) 
