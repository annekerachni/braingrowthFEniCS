...
def FA_to_tangential_growth_law_linearrelationship(coef, normalized_fa_nodal_value):
    # linear. alphaTAN = a * FA --> FA ++ => growth ++
    alphaTAN_nodal_value = coef * normalized_fa_nodal_value
    
    return alphaTAN_nodal_value


def FA_to_tangential_growth_law_exponential_minusFA_Relationship(coef, normalized_fa_nodal_value):
    # alphaTAN = a * exp(-FA) --> FA -- (like in central sulcus at 28GW) => growth ++
    alphaTAN_nodal_value = coef * math.exp(-normalized_fa_nodal_value)
    
    return alphaTAN_nodal_value

def FA_to_tangential_growth_law_1_over_1_plus_FA_Relationship(coef, normalized_fa_nodal_value):
    # alphaTAN = a * exp(-FA) --> FA -- (like in central sulcus at 28GW) => growth ++
    alphaTAN_nodal_value = coef * (1 / (1 + normalized_fa_nodal_value) )
