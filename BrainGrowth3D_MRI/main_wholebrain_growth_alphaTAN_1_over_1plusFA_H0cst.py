...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output', help='Output folder path', type=str, required=False, 
                        default='./results/ECCOMAS_57/wholebrain/9SEPT/dHCPsurface21GW_wholebrain_Fgt_quasistatic_alphaTANFAbased_1_over_1plusFA_H0cst_DIRICHLETboundaries_GAP_dgRAD_calibration/')
    ...
    list_alphaTAN_exp_minus_normalizedFAbased = [] # Sum(alphaTAN_FA)_{n_nodes_in_Cortex}
    for scalarDOF in list_of_scalarDOF_in_Cortex:
        list_alphaTAN_homogeneous.append(alphaTAN_homogeneous * gm.vector()[scalarDOF])
        list_alphaTAN_exp_minus_normalizedFAbased.append( alphaTAN_normalizedFA.vector()[scalarDOF] )
    
    coef_lissage_alphaTAN_FA = np.sum(list_alphaTAN_homogeneous) / np.sum(list_alphaTAN_exp_minus_normalizedFAbased)
