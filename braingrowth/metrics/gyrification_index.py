...
if __name__ == '__main__':
    ...
    default='./data/dhcp/dhcp_atlas/21GW/dhcp21GW_masked_29296faces_smoothed.stl') # taken as the convex hull mesh
    ...
    for numerical_time in args.numericaltimes:
        ...
        folded_mesh_path_STL = './simulations/dhcp/dhcp_experiment_nsteps{}/dhcp_experiment_nsteps{}_time{}.stl'.format(args.nsteps, args.nsteps, str(numerical_time).split('.')[0] + "_" + str(numerical_time).split('.')[-1])
        folded_mesh = meshio.read(folded_mesh_path_STL)
        
        if args.visualizationmode == True:
            # plot folded mesh
            folded_mesh_XMLpath = './simulations/dhcp/dhcp_experiment_nsteps{}/dhcp_experiment_nsteps{}_time{}_volume.xml'.format(args.nsteps, args.nsteps, str(numerical_time).split('.')[0] + "_" + str(numerical_time).split('.')[-1])
        ...
        if args.visualizationmode == True:
            ...
            rescaled_folded_mesh_XMLpath = './simulations/dhcp/dhcp_experiment_nsteps{}/dhcp_experiment_nsteps{}_time{}_volume_RESCALED.xml'.format(args.nsteps, args.nsteps, str(numerical_time).split('.')[0] + "_" + str(numerical_time).split('.')[-1])
