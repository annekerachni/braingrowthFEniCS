...
def transfer_nifti_values_to_FEnicSmesh_nodes(MRI_nifti_path, MRIniftiname, XML_mesh_path):
    """
    ...
    # ------------------------
    mesh = fenics.Mesh(XML_mesh_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--inputmesh', help='Path to input FEniCS readable formal .xml mesh)', type=str, required=False, 
                        ...
                        # './data/dhcp/refined/dhcp21GW_17K_refined10.xml'
    ...
    parser.add_argument('-o', '--output', help='Path to output folder where to write mesh + loaded nifti values (.xdmf))', type=str, required=False, 
                        default='./utils/nifti_mesh/niftitomesh/transfer_niftivalues_to_meshnodes/results_FEniCScode/')
    ...
    # FEniCS mesh path (.xml)
    XML_mesh_path = args.inputmesh
    mesh = fenics.Mesh(XML_mesh_path)
    
    # Load MRI nifti values onto mesh nodes (get n_nodes numpy array)
    regions_T2, functionFEM_T2 = transfer_nifti_values_to_FEnicSmesh_nodes(nifti_path_T2, "T2", XML_mesh_path)
    regions_segmentation, functionFEM_segmentation  = transfer_nifti_values_to_FEnicSmesh_nodes(nifti_path_segmentation, "Segmentation", XML_mesh_path) 
    regions_FA, functionFEM_FA = transfer_nifti_values_to_FEnicSmesh_nodes(nifti_path_FA, "FA", XML_mesh_path)
    
    # Store regions in XDMF
    #######################
    # T2
    # --
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_T2_21GW.xdmf")
    xdmf.write(functionFEM_T2)
    xdmf.close()
    
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_T2_21GW_regions.xdmf")
    xdmf.write(regions_T2)
    xdmf.close()
    
    # Segmentation
    # ------------
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_Seg_21GW.xdmf")
    xdmf.write(functionFEM_segmentation)
    xdmf.close()
    
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_Seg_21GW_regions.xdmf")
    xdmf.write(regions_segmentation)
    xdmf.close()
    
    # FA
    # --
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_FA_21GW.xdmf")
    xdmf.write(functionFEM_FA)
    xdmf.close()
    
    xdmf = fenics.XDMFFile(mesh.mpi_comm(), args.output + "mesh_FA_21GW_regions.xdmf")
