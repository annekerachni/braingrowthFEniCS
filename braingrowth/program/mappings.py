...
def mesh_to_surface_V(mesh_vertex_coords, bmesh_vertex_coords):
    ...
    """ vertexWholeMeshIDX_to_projectedVertexBoundaryMeshCOORDS_mapping = [] """
    ...
    for vertex_index, coords in enumerate(mesh_vertex_coords): #N vertex indices in the whole Mesh ref
        ...
        """
        closest_point_on_boundary_COORDS = bmesh_vertex_coords[closestBPointIndex_inBref] # Get associated coordinates on projected vertex 
        vertexWholeMeshIDX_to_projectedVertexBoundaryMeshCOORDS_mapping.append(closest_point_on_boundary_COORDS) # build mapping: vertex IDX i in whole mesh --> projected (onto boundary mesh) vertex COORDS  
        """
    ...
    """ vertexWholeMeshIDX_to_projectedVertexBoundaryMeshCOORDS_mapping = np.array(vertexWholeMeshIDX_to_projectedVertexBoundaryMeshCOORDS_mapping) """
