...
class BrainSurface(fenics.SubDomain): 
    ...
    def inside(self, x, on_boundary): # does not need to be called (FEniCS specific class function to define boundaries)
        _, distance = self.brainsurface_bmesh_bbtree.compute_closest_entity(fenics.Point(*x)) # compute_closest_point() https://fenicsproject.org/olddocs/dolfin/1.5.0/python/programmers-reference/cpp/mesh/GenericBoundingBoxTree.html
